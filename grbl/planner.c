...
uint8_t plan_buffer_line(float *target, plan_line_data_t *pl_data)
{
  ...
  if (block->condition & PL_COND_FLAG_SYSTEM_MOTION) { 
    ...
    block->steps[B_MOTOR] = labs(sqrt(square(settings.max_travel[X_AXIS]*settings.steps_per_mm[A_MOTOR]-(target_steps[X_AXIS]+position_steps[X_AXIS])) + square(target_steps[Y_AXIS]-position_steps[Y_AXIS])));
  #endif
